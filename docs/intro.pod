...
Square brackets (C<[> and C<]>) delimit standard Deck
...
The compiler will never generate a getter or setter if you've
...
C<atPut> (not shown) takes an index and a value and stores the
