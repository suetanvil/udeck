...
closure (in C<ff>) and calls it repeatedly to generate each new number
in the Fibonacci sequence.  The state (the current next Fibonacci
number) is stored in those variables which are now only visible to the
closure.
