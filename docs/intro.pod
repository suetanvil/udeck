...
As mentioned previously, lists in Deck are not linked lists the way
they are in most Lisp dialects.  Instead, they are arrays (aka
vectors) of the sort you see in most programming languages.  Since
Lists are objects and Deck offers other list-like types (Strings and
ByteArrays for example), it makes sense for list-like objects to have
a common interface and for the system to use them.  They do and this
interface is known as the B<Sequence Protocol>.
