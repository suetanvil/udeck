...
detail below in the section L<Objects>.
...
The two are currently different names for the same macro.

For ordinary variable assignment, it expands to the internal
primitive assignment statement.  Since this is implementation defined,
that's really all I need to tell you about it.  However, in the
interest of transparency, I'll explain more.

The internal assignment function is (currently) named C<_::set>.  It
takes two arguments, a symbol naming the destination variable and the
value to assign to it.  This behaves pretty much as expected.
However, the first argument B<must> be a symbol literal.  This way,
the compiler can catch undeclared variables.

If the destination of an assignment is an array element or an object
attribute, the assignment macro expands the expression into an
C<atPut> call or an attribute call, respectively:

	a@42 = 5		# Becomes: a->atPut 42 5
	b.foo = 5		# Becomes: b->foo_set 5
...
Variables and constants declared at the toplevel are global (at least
to the package in which they occur).
...
Read-only sequences do not implement C<atPut>.
...
implementation that returns C<false>.
...
Names defined in a package can be imported into the current package
either en masse or in a controlled manner.


=head2 Defining and Loading Packages

A package is a Deck source file. 

A C<use> statement takes the symbol following it and if there is no
corresponding namespace yet, creates a filename from it and loads the
file.  The file's first statement must be a C<package> declaration
followed by a symbol and that symbol B<must> be identical to the
symbol given to the C<use> statement.

The argument to C<use> is converted to a string, all instances of
'::' are converted into directory separators and the extension ".dk"
is appended.  Thus,

	use Database::SQL::MySQL

attempts to load "Database/SQL/MySQL.dk".  

The global list C<Sys::ModPath> contains the directories the compiler
will search for.  The compiler searches each directory in this list
until it finds a source file with the matching name.  It is a fatal
error if it does not find anything.

It then loads the source file.  The first statement of the input file
must be a C<package> declaration and the word following C<package>
must be identical to the word following the C<use> statement.
...
Deck provides a single, flat global namespace.  All names take the
form:
...
=head2 Importing Packages

=head3 with, without and rename
