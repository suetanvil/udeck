...
Secondly, the C<MethodCall> gets called like a procedure (it is a
callable object so that works).  
...
Variables may also be made publically visible via attributes, either
by hand-coding them or via the C<public>, C<readable> and C<writeable>
words.
...
=head3 Basic Classes and Struct
