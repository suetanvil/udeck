...
A line runs from either the start of the input file B<or> from the
character after the end of the previous line through to the next ASCII
newline character B<or> the end of the file.  For example:
...
shells), the backslash is not an escaped newline.  You may put other
whitespace or a comment after a backslash and it will continue to mean
the same thing:
...
=head2 POD Regions, lack thereof

By convention, Deck documentation is written in POD format.  This
includes the library docstrings, which are extracted via a utility and
them passed to pod2html et. al. for processing.
...
C<{> and C<}>, the curly brackets

=item 2.

C<(> and C<)>, the round brackets

=item 3.

C<[> and C<]>, the square brackets

=item 4.

C<:>, the quote delimiter

=item 5.

C<=> at the start of a LoL line indicates the line should be parsed as infix.
...
The equal symbol (C<=>) affects the parsing of LoL lines.  See <LoLs>
below.
...
parser (see B<Expressions> below).  For example:
...
C<Lang::value> with the element as an argument:

    :(a) == :[Lang::value a]

This guarantees that an infix expression always expands into a
function call that, when evaluated, returns the expression's value.
...
if it consists of a series of terms divided by boolean AND (C<&&>) and
OR (C<||>) operators.

In addition, you can force a line to be parsed as infix by prefixing
it with an equal symbol (C<=>).  This will be discarded and the rest
...
=item 1. Contain an odd number of elements.
...
Any LoL line beginning with a single C<=> will be parsed as infix.
...
=head3 Compact Sub Declarations
...
a list of arguments delimited by C<|> before and after:
...
In fact, the parser converts the former into (more or less) the latter
at compile time.  It is possible to view the expression using the
quote operator.  For example, this:

    puts :{|| = 42}

will display the resulting expression.

(See 'Closures' below for an explanation of C<sub>.)
...
Parameter names (the words between the C<|> symbols) may not be operators.

=item 2.

Any number of spaces or newlines may be used between tokens up to the last C<|>.
...
is the last element of a C<sub>; all other cases may trigger a
...
=item 1) One or more words, including escaped operators.
...
following the word C<var> or C<const> are treated identically to other
...
The only difference is that you can't use 'set' instead of the '=' in
case 2), as something like this:
...
infix by the macro implementing C<var> or C<const>.  One consequence
...
stripped out before the C<var> macro can parse the argument.  Oddly
enough, this:

	var \+ = 42

will work (at least at the time of this writing--don't depend on it).
Of course, you can always use C<var {\= = 42}> instead.

There is a special parsing rule that lets C<var> and C<const> get away
with the unescaped C<=> in what is technically a prefix expression.

It is possible to take the result of a variable declaration.  However,
...
The two are equivalent (and in fact are just different names for the
same macro) but the former is generally parsed as infix.  The
...
case, the RHS B<must> evaluate to a list-like object (i.e. one that
implements the method C<at>).  Each variable in the LHS is assigned
...
The RHS must have at least as many elements as the LHS but may have
more.  This restriction is checked at run time.



=head3 Docstrings

Packages, procedures, methods and classes may optionally begin with a
single string literal (B<Beware:> interpolated strings don't count.)
If present, this string literal and other information about the object
it annotates are stored in the interpreter.
...
it is visible to the final block but has a value of C<nil>.
...
C<name> may also be omitted.  In this case, C<proc> will define the
proc but will not bind it to a global name.  However, C<proc> always
returns the resulting proc and so it can be assigned this way.  For
example, the following to expressions are roughly identical.

    proc foo {a b} {return (a + b)}
    
    const foo = [proc {a b} {return (a + b)}]

Procs all define const C<return> when called.  It is bound to the
...
passed as the argument.  C<number> is the number of formal arguments.
Arguments are named C<a> through C<z>.  More than 26 arguments is an
error.

If the argument is not a LoL, it is passed unmodified, unless the
argument is prefixed with C<strict> in which case it is a fatal error.

(Note: earlier versions of Deck also allowed a list containing a
single expression.  This is no longer allowed.  Finding a non-list
entry in a sub argument will disqualify it from being converted to a
sub.)
...
such names into the current namespace.  For example, this will B<not
work>:
...
accomodate the limits of the filesystem.

=head3 Package Docstrings

If the following statement is a string literal, it is treated as a
docstring for the package.  For example:
...
=head3 Running Packages as Scripts
...
script.  If the interpreter finds a C<package> declaration at the
start of the program, it:

=over

=item 1) Sets the namespace to the package namespace.

=item 2) Sets the global variable C<Sys::PackageRun> to true.

=item 3) Reads the docstring if present.

=item 4) Executes the rest of the source file as usual.

=back

No filename checks are done in this case.  The source file name does
not need to be related to the package name.

The global C<Sys::PackageRun> is useful for detecting when a package
