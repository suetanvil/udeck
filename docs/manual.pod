...
C<arg-list> is the list of formal arguments.  It must be a list
containing symbols, each the name of a procedure argument.
...
The C<proc> macro automatically smoothes out the differences between
the list formats.  

C<body> is a quoted list of lists (I.e. a LoL) containing the
expressions which are actually evaluated when the procedure is
invoked.

C<final> is the final block (explained below).  It is optional.
