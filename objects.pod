...
X	4) replace all uses of ' class' with calls to 'deckClass'.
		-make class() store classes externally to the namespace.
	5) Move relevant stuff into Object

	6) Let declarations in Lang.dk modify base class definitions.
		-implement _::sys_class to represent the exact semantics
		 of modifying an existing builtin class.
